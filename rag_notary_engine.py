...
def _ask_claude(system: str, user: str, max_tokens: int = 1500) -> str:
    ...
    if not state.claude:
        raise HTTPException(
            status_code=503,
            detail="Claude не подключён. Добавь ANTHROPIC_API_KEY в .env"
        )
...
async def check_plagiarism(
    ...
    if state.clip is None or state.processor is None:
    ...
    try:
        ...
        log.exception("Ошибка открытия изображения")
        raise HTTPException(400, f"Не удалось открыть изображение: {e}")

    # 2. Сравнение через CLIP
    try:
        with torch.no_grad():
            inputs_a = state.processor(images=img_a, return_tensors="pt")
            inputs_b = state.processor(images=img_b, return_tensors="pt")

            # vision_model возвращает чистый тензор без обёртки
            feat_a = state.clip.vision_model(**inputs_a).pooler_output
            feat_b = state.clip.vision_model(**inputs_b).pooler_output
        ...
        raise HTTPException(500, f"CLIP ошибка: {e}")

    # 3. Формирование вердикта
    pct = round(score * 100, 2)
    if score >= 0.95:
        verdict, details = "DUPLICATE", f"Изображения идентичны ({pct}%)."
    elif score >= 0.80:
        verdict, details = "SUSPECT", f"Высокое сходство ({pct}%) — рекомендуется проверка."
    else:
        verdict, details = "ORIGINAL", f"Изображения различны ({pct}%). Плагиат маловероятен."
